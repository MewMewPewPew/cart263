window.onload = function(){
    ...
    let filterButton_blur = this.document.getElementById("blurnum");
    ...
    //add the following BEFORE the context.drawImage() instruction.
    context.filter = `blur(${userProvidedBlur}px)`;
    context.filter += `hue-rotate(${userProvidedDeg}deg)`;
    context.drawImage(video, 0, 0, canvas.width, canvas.height); //video element referenced (used like an img)
